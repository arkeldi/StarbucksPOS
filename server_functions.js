...
function GetXReport() {
  ...
  useEffect(() => {
    async function fetchXReport() {
      const response = await fetch(`http://localhost:3001/XReport`);
      const data = await response.json();
      setXReport(data);
      //console.table(data);
    }
...
function GetZReport() {
  ...
  useEffect(() => {
    async function fetchZReport() {
      const response = await fetch(`http://localhost:3001/GetZReport`);
      const data = await response.json();
      setReport(data);
      //console.table(data);
    }
...
function GenerateZReport() {
  async function handleSubmit() {
    // console.log(info[0])
    const response = await fetch(`http://localhost:3001/GenerateZReport`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
      })
...
function GetRestockReport() {
  ...
  useEffect(() => {
    async function fetchRestocks() {
      const response = await fetch(`http://localhost:3001/restockReport`);
      const data = await response.json();
      setRestocks(data);
      //console.table(data);
    }
...
function GetExcessReport(start, end){
  ...
  useEffect(() => {
    async function fetchExcessList() {
      const response = await fetch(`http://localhost:3001/excessReport/${start}/${end}`);
      const data = await response.json();
      setExcessList(data);
      //console.table(data);
    }
...
function GetSoldTogether(start, end) {
  ...
  useEffect(() => {
    async function fetchSoldTogether() {
      const response = await fetch(`http://localhost:3001/soldTogether/${start}/${end}`);
      const data = await response.json();
      setSold(data);
      //console.table(data);
    }
...
export {GetInventoryList, GetCurrentInventoryList, GetProductsList, GetCustomizationsList, GetSalesList, GetOrdersList, GetEmployeesList
  ...
  AddtoRecipes, GetSalesReport, GetXReport, GetZReport, GetExcessReport, GetRestockReport, GetSoldTogether, GenerateZReport, AddtoSales
};
